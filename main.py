def place_order(menu):
    ...
    while True:
        ...
        for category in menu:
            for option in menu[category]:
                print_menu_line(i, category, option, menu[category][option])
        ...
        update_order(order, item_index, menu_items)

        continue_flag = input("\nWould you like to place the order? (Y/N): ")
        while continue_flag.lower() not in "y n".split(" "):
            continue_flag = input("Invalid input! Please enter Y or N: ")

        if continue_flag.lower() == "n":
            print("\nThank you for shopping with us!")

            prices_list = [item["price"] * item["quantity"] for item in order]

            total = round(sum(prices_list), 2)
            break
...
def update_order(order, menu_selection, menu_items):
    ...
    """
    if menu_selection.isdecimal() and int(menu_selection) in menu_items:
        item = menu_items[int(menu_selection)]

        print(f"How many {item["Item name"]}s would you like to order?")
        quantity = input("> ")
...
Input [{menu_selection}] is invalid.
Please try again.
""")
    return order
